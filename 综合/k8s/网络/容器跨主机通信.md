...
* vxlan    
 flanneld 进程在 Node 2 节点启动时，自动添加在 Node 1 上ARP 记录，并且还会维护FDB的数据    
 Flannel会将需要传递的IP 包封装进行**二层封包工作**，封装目的 VTEP 设备”的 MAC 地址。  
 为了标识数据帧是VXLAN的数据帧，需要添加**VXLAN 头**，为了能在宿主机传送，需要再封装成为UDP包（内核态）  
