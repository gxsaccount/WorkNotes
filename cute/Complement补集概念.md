...
> **`A*` = 让「A 的副本」无重叠铺满 M 的那组基址。**
>
> **即：`A 的像 + A* 的像 = {0, 1, ..., M-1}`，且一一对应（双射）。**
>
> **A\* 的每个值 = 一个副本的起点；`size(A*)` = 副本个数；`cosize(A)` = 副本间距。**
...
**"铺满 + 无重叠" = 双射** —— 这正是 divide 结果是 permutation（而非单纯 gather）的根本原因。
...
| 7 | **composition 只得到部分元素，divide 得到全部？** | **对**。差别在定义域：`composition(A,B)` 定义域 = `size(B)`（1 个 tile）；`divide(A,B)` = `size((B,B*))` = `size(A)`（全部）。元素没丢，是前者"够不着" |
...
**单靠 B 只够得着 1 个 tile；补上 B\* 的 6 个起点才覆盖全部 —— 这就是 `B*` 不可替代的原因。**
...
**② divide 自动保证单射** —— `(B,B*)` 的像集无重叠铺满 `0..M-1` → 天然双射。
（对比：单独 `A ∘ B` 时 B 可能非单射，如 F3 的 `(2,4):(2,1)` —— 那时只是 gather 不是 permutation；divide 补上 `B*` 后自动避开此问题。）
