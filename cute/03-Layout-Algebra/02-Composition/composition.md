...
>
> 更新：2026-09-15 —— 将 `/d` 与 `%s` 改为同一个 shape/stride 联合例子，避免脱离 stride 单独罗列 shape。
...
def slash_d(lay, d):
    ...
    for s_i, d_i in zip(lay.shape, lay.stride):
        ...
        st.append(d_i * rem)              # stride × 当前剩余的 d（residue）
        rem //= f
        if rem == 1:
...
真正的不变量是采样语义：若结果记作 `A_d`，则 **`A_d(k) = A(d*k)`**。

### shape 和 stride 一起算

不要只看 shape。沿用后面完整 composition 的例子：
...
先算 `A / 9`。从左到右把 `9` 分解到 A 的各个 mode：

| mode | 原 `shape:stride` | 当前 residue `r` | 消掉因子 `f` | 新 `shape:stride` | 逐层验算 |
|---|---|---:|---:|---|---|
| 0 | `3:w` | `9` | `3` | `1:(9w)` | `3=1×3`，`9w=w×9`，下一层 `r=9/3=3` |
| 1 | `6:x` | `3` | `3` | `2:(3x)` | `6=2×3`，`3x=x×3`，下一层 `r=3/3=1` |
| 2 | `2:y` | `1` | `1` | `2:y` | `2=2×1`，`y=y×1` |
| 3 | `8:z` | `1` | `1` | `8:z` | `8=8×1`，`z=z×1` |
...
这张表要成对看，但 shape 和 stride **不是机械地乘除同一个局部因子**：

- shape 除以本 mode 消掉的部分 `f`
- stride 乘以进入本 mode 时尚未消掉的采样距离 `rem`
- 然后更新 `rem = rem / f`

各层的 `rem`（这里是 `9,3,1,1`）就是官方所说的 **residues**。

逐 mode 验算时，不要笼统检查变换前后的 `shape×stride` 是否相等。对进入当前
mode 的 residue `r`，应检查：

```text
原 shape  = 新 shape × 本层消掉因子 f
新 stride = 原 stride × 当前 residue r
下一层 r  = 当前 r / f
```

所有 mode 处理完后，还应确认最终 `r=1`；否则说明采样间隔没有被完整分解。

从序列角度看，新 layout 的第 `k` 个元素就是原 layout 的第 `9k` 个元素，因此 `/9` 同时改变 shape 和 stride，而不是只做一次 shape 除法。

### 用循环看 `/9`

原 layout 是四层循环，mode-0 最内层：
...
- `i0` 每次变化 1，所以逻辑步长是 `1`
- `i1` 每加 1，要越过 `3` 个 `i0`
- `i2` 每加 1，要越过 `3×6=18` 个内层坐标
- `i3` 每加 1，要越过 `3×6×2=36` 个内层坐标

所以坐标 `(i0,i1,i2,i3)` 的逻辑 index（n）是：

```text
n = i0 + 3*i1 + 18*i2 + 36*i3
```

也可以因式分解成 `i0 + 3*(i1 + 6*(i2 + 2*i3))`。它只由 **shape** 和循环顺序决定；真正的内存 offset 仍是 `w*i0 + x*i1 + y*i2 + z*i3`。

`/9` 表示只执行 `n = 0, 9, 18, 27, ...` 对应的迭代。因为：

```text
9 = 3 × 3
    └─ 先跨过完整的 i0 循环（shape 3）
        再让 i1 每次前进 3 格
```

从逻辑 index `n` 反算原坐标时，按 shape 从左到右连续做除法和取余：
...
| 新 index `k` | 原 index `n=9k` | 原坐标 `(i0,i1,i2,i3)` | offset |
|---:|---:|---|---|
| 0 | 0  | `(0,0,0,0)` | `0` |
| 1 | 9  | `(0,3,0,0)` | `3x` |
| 2 | 18 | `(0,0,1,0)` | `y` |
...
上面的表还不足以直接推出循环，需要再把新 index `k` 拆成新坐标。先明确新 shape
是怎么得到的：

```text
原 shape = (3,6,2,8)，采样间隔 d = 9

mode-0：用 shape 3 消掉 9 中的因子 3
        shape 3 / 3 = 1，剩余采样间隔 9 / 3 = 3

mode-1：再消掉剩余的因子 3
        shape 6 / 3 = 2，剩余采样间隔 3 / 3 = 1

mode-2、mode-3：采样间隔已经变成 1，shape 保持不变

所以 /9 后的完整 shape = (1,2,2,8)
```

第一个 mode 的 shape 为 `1`，循环变量只能取 `0`，删除这个退化 mode 后，有效 shape
才是 `(2,2,8)`：
...
所以原 layout 在第 `9k` 个逻辑坐标上的 offset 为：
...
for (j3 = 0; j3 < 8; ++j3)
  for (j2 = 0; j2 < 2; ++j2)
    for (j1 = 0; j1 < 2; ++j1)
      offset = 3*x*j1 + y*j2 + z*j3;
```

这个有效循环对应 coalesce 后的：

```text
(2,2,8):(3x,y,z)
```

CuTe 的 `/9` 中间结果会先保留原来的 mode 层级，写成：

```text
(1,2,2,8):(9w,3x,y,z)
```

其中 `j0` 的范围只有 `0..0`，所以 `9w*j0` 永远是 `0`。因此，**仅从运行结果无法反推出
shape-1 mode 的 stride 是 `9w`**；`9w` 来自 `/` 运算保留的形式化 residue。删除这个退化
mode 后，才得到上面循环实际表达的 `(2,2,8):(3x,y,z)`。

> `/9` 跳过的是 **9 个逻辑坐标**，不是让内存 offset 每次固定增加 9。实际 offset 的变化仍由 `(w,x,y,z)` 决定。
...
现在做 `%16`，从左到右保留能够组成前 16 个元素的 shape：

| mode | 输入 `shape:stride` | 保留后的 `shape:stride` | 剩余数量 |
|---|---|---|---:|
| 0 | `1:9w` | `1:9w` | `16 / 1 = 16` |
| 1 | `2:3x` | `2:3x` | `16 / 2 = 8` |
| 2 | `2:y` | `2:y` | `8 / 2 = 4` |
...
```text
((3,6,2,8):(w,x,y,z) / 9) % 16
    = (1,2,2,4):(9w,3x,y,z)
```

这里 `%16` **只截短 shape，stride 原样保留**。把两步连起来看，就是：
...
**分支 1 `A ∘ 4:3`**
```
/3 → (6/3, 2):(8*3, 2) = (2,2):(24,2)
     这里 d=3 完全由 mode-0 吸收，所以 mode-1 的 2:2 原样保留
     逐层验算：6=2×3，24=8×3，下一层 residue=3/3=1
     验证采样语义：结果序列 [0,24,2,26] = [A(0),A(3),A(6),A(9)]
%4 → size = 4 = s，恒等
→ (2,2):(24,2)
```
...
> **把 A 看成一个数组：**
> `/d` 是按逻辑 index **每隔 d 个采样一次**（将 d 逐 mode 分解，并用各层 residue
> 缩放 stride，使结果满足 `A_d(k)=A(d*k)`，受 stride divisibility condition 约束）；
...
**Composition 可以理解为** loop fusion + strength reduction：把两级坐标映射合成为一级。
当 Shape/Stride 全部静态时，`constexpr` 使这些化简通常可在编译期完成；若包含动态值，
则仍可能保留运行时的坐标运算，不能笼统宣称“运行时零开销”。
...
python3 03-Layout-Algebra/02-Composition/代码实验/sim2.py
python3 03-Layout-Algebra/02-Composition/代码实验/sim3.py
